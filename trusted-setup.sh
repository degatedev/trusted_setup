...
echo "Step 1 : Downloading and Compile source code" 2>&1 | tee >> /opt/trustmount/trusted-setup.log
...
echo "Step 2 : Downloading the previous contribution zip file from sftp server" 2>&1 | tee >> /opt/trustmount/trusted-setup.log
...
#Start contribution
echo "Step 3 : Starting contribution, this process takes a long time, about 20 hours" 2>&1 | tee >> /opt/trustmount/trusted-setup.log
...
echo "Step 4 : Uploading contribution file to sftp server" 2>&1 | tee >> /opt/trustmount/trusted-setup.log
cd ${file_path} && /usr/bin/sftp -o StrictHostKeyChecking=no  -i ${mount_path}/sftpuser${contribution_number}.key  sftpuser${contribution_number}@trust-setup-ftp.degate.com <<EOF
...
if [ ! -n "$exit_file" ];then
    echo "Step 4 : Upload contribution file error, please contact coordinator" 2>&1 | tee >> /opt/trustmount/trusted-setup.log
fi
